...
public class Scrabble{
    ...
    public static String longestWord(char availableLetters[], String prefix) {
   	 ...
   	 if (myDictionary.contains(prefix)){  // checks if prefix sent from recursion is in dictionary
   		 longest= prefix;
   	 }
   	
   	
   	 for(int i=0; i<availableLetters.length+1;i++){  // adds a single letter from the array given (loop through all of them)
   	 ...
   	 if( availableLetters2.length != 0){        //recursion step , sends the prefix with the added letter back to the beginning of the method
